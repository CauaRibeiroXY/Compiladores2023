...
Expressao* Expressao::extrai_expressao(No_arv_parse* no) {
  // 23) a 48)
  switch(no->regra) {
  case 23: { // 26) EXPRESSION_ART -> EXPRESSION_ART + TERM
    cout << "Soma" << endl;
  ...
  case 24: { // 27) EXPRESSION_ART -> EXPRESSION_ART - TERM
  ...
  case 25: { // 25) EXPRESSION_ART -> TERM
    cout << "Term" << endl;
    return extrai_expressao(no->filhos[0]);
  }
  case 27: { // 27) TERM -> TERM * FACTOR
  ...
  case 37: { // 37) FACTOR -> FLOAT_LITERAL
    ExpressaoFloat* res = new ExpressaoFloat();
    res->numero = FLOAT::extrai_FLOAT(no->filhos[0]);
    cout << "Numero " << endl;
    return res;
  }
  case 38: { // 38) FACTOR -> ( EXPRESSION_LOGICA )
    return extrai_expressao(no->filhos[1]);
  }
  case 39:{ // 39)EXPRESSION_COMP -> EXPRESSION_ART == EXPRESSION_ART
    ExpressaoIgual* res = new ExpressaoIgual();
    res->esquerda = extrai_expressao(no->filhos[0]);
    res->direita = extrai_expressao(no->filhos[2]);
    return res;
  }
  case 40:{// 40)EXPRESSION_COMP -> EXPRESSION_ART < EXPRESSION_ART
    ExpressaoMenor* res = new ExpressaoMenor();
    res->esquerda = extrai_expressao(no->filhos[0]);
    res->direita = extrai_expressao(no->filhos[2]);
    return res;
  }
  case 41:{// 41) EXPRESSION_COMP -> EXPRESSION_ART

    return extrai_expressao(no->filhos[0]);

  }
  case 42 :{// 42) EXPRESSION_LOGICA && EXPRESSION_COMP
    ExpressaoAnd* res = new ExpressaoAnd();
    res->esquerda = extrai_expressao(no->filhos[0]);
    res->direita = extrai_expressao(no->filhos[2]);
    return res;
  }
  case 43:{// 43) EXPRESSION_LOGICA -> EXPRESSION_LOGICA || EXPRESSION_COMP
    ExpressaoOr* res = new ExpressaoOr();
    res->esquerda = extrai_expressao(no->filhos[0]);
    res->direita = extrai_expressao(no->filhos[2]);
    return res;
  }
  case 44:{// 44) EXPRESSION_LOGICA -> ! EXPRESSION_COMP
    ExpressaoNegacao* res = new ExpressaoNegacao();
    res->direita = extrai_expressao(no->filhos[1]);
    return res;
  }
   case 45:{// 45) EXPRESSION_LOGICA -> EXPRESSION_COMP
